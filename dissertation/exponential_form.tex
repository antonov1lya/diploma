...
$$
    \Prb(X=x,Y=y,Z=z) = p_{000}^{(1-x)(1-y)(1-z)} \ldots p_{111}^{x y z} =
$$
$$
    =
...
\begin{theorem}\label{at_least_zero}
    ...
    \end{proof}
\begin{consequence}
    Из \autoref{at_least_zero} непосредственно следует, что
    ненулевое значение параметра
    $\theta = \ln  \left(\dfrac{p_{001}p_{111}p_{010}p_{100}}{p_{011}p_{101}p_{000}p_{110}}\right)$ является достаточным условием
...
\end{consequence}
    ...
    % воспользуемся 
    % теорией РНМН тестов \cite{Lehmann1986} в многопараметрическом 
    ...
    $$
    \Prb(X_1=x_1,Y_1=y_1,Z_1=z_1,\ldots,X_n=x_n,Y_n=y_n,Z_n=z_n)=
    $$
    $$
    =\prod_{i=1}^n \Prb(X_i=x_i,Y_i=y_i,Z_i=z_i) =
    $$
    $$
    ...
    \begin{lemma}\label{u_dist}
        ...
        Тогда
        $$\Prb_{\theta=0}(U=u \mid T=t)=\dfrac{(\prod_{i=1}^8 k_i(u)!)^{-1}}
    ...
    \begin{proof}
        ...
        $$
            \Prb(U=u,T=t)=\Prb(U=u, T_1=t_1, T_2=t_2, T_3=t_3, T_4=t_4, T_5=t_5, T_6=t_6)=
        $$
        $$
            =\Prb\biggl(\sum_{i=1}^n X_i Y_i Z_i=u, \sum_{i=1}^n X_i Y_i=t_1, \sum_{i=1}^n X_i Z_i=t_2,\sum_{i=1}^n Y_i Z_i=t_3,
            $$
            $$
            \sum_{i=1}^n X_i=t_4,\sum_{i=1}^n Y_i=t_5, \sum_{i=1}^n Z_i=t_6\biggr)=
        $$
        $$
            =\Prb\biggl(\sum_{i=1}^n X_i Y_i Z_i=u, \sum_{i=1}^n X_i Y_i (1- Z_i)=t_1-u, \sum_{i=1}^n X_i (1-Y_i) Z_i=t_2-u,
        $$
        $$
        ...
        $$\Prb(U=u \mid T=t)=\dfrac{\Prb(U=u,T=t)}{\Prb(T=t)}=
        \dfrac{\Prb(U=u,T=t)}{\sum_{s \in \mathcal{D}} \Prb(U=s,T=t)}=
        $$
        $$
        ...
        При истинности гипотезы $\theta=0$ справедливо равенство $\dfrac{p_{001}p_{010}p_{100}p_{111}}{p_{000}p_{011}p_{101}p_{110}}=1$. Следовательно:
        $$\Prb_{\theta=0}(U=u \mid T=t)=\dfrac{(\prod_{i=1}^8 k_i(u)!)^{-1}}
            {\sum_{s\in \mathcal{D}} (\prod_{i=1}^8 k_i(s)!)^{-1}}$$
    \end{proof}
